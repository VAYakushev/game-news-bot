...
class VKPoster:
    ...
    def rewrite_with_llm(self, title, description):
        ...
        prompt = f"""Ты — редактор новостного канала про игры. На основе короткого описания новости напиши связный абзац для поста в группе.

Оригинальное описание: {description}

Правила:
- Пиши 3-4 предложения связным текстом, как одна осмысленная мысль
- Используй союзы и переходы между предложениями (кроме того, также, при этом, как отмечают эксперты)
- Не перечисляй факты через запятую — рассказывай историю
