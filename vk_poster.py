...
class VKPoster:
    ...
    def rewrite_with_llm(self, title, description):
        ...
        prompt = f"""Ты — редактор новостного канала про игры. На основе короткого описания новости напиши развёрнутое вступление для поста в группе.

Оригинальное описание: {description}

Напиши 3-4 предложения, которые:
- Добавляют больше деталей и контекста
- Могут включать дополнительную информацию (дата, причины, реакция)
- Не повторяют заголовок
- Пиши на русском, без эмодзи"""
