...
class VKPoster:
    ...
    def rewrite_with_llm(self, title, description):
        ...
        prompt = f"""Ты пишешь пост для группы VK про игры. Напиши связный абзац на основе фактов.

Факты: {description}

ВАЖНО: Пиши СВЯЗНЫЙ ТЕКСТ с переходами между предложениями. НЕ перечисляй факты через запятую.

ПЛОХО (так НЕ надо писать):
"Продажи превысили 50 тысяч копий, положительные отзывы в Steam, исследование заснеженного мира, бесплатное обновление"

ХОРОШО (так надо писать):
"Игра набрала более 50 тысяч продаж и получила положительные отзывы в Steam. Игроки отмечают атмосферное исследование заснеженного мира, а разработчики уже анонсировали бесплатное обновление с кооперативным режимом."

Напиши 3-4 предложения связным текстом. Без эмодзи, на русском."""
